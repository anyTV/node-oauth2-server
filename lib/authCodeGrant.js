...
function redirect (done) {
  var url,
    state = this.req.body.state || this.req.query.state;

  if(this.responseType === 'code') {
    url = this.client.redirectUri +
        (/[?|&]([a-zA-Z0-9\-]+)=/.test(this.client.redirectUri) ? '&' : '?') +
        'code=' +
        this.authCode;
  } else {
    url = this.client.redirectUri + '#token=' + this.accessToken + (state ? '&state=' + state : '');
  }
